...
export function DatabaseTest({ currentUser }) {
  ...
  // Generate query for any table
  const getQueryForTable = (tableName) => {
    if (testQueries[tableName]) {
      return testQueries[tableName]
    }
    // Default query for any table not in predefined list
  ...
  const getSortedData = () => {
    ...
    return [...testResult.data].sort((a, b) => {
      ...
      // Try to parse as numbers if both values look numeric
  ...
  const runTest = async () => {
    console.log('=== Run Test Query Button Clicked ===')
    ...
    try {
      ...
      console.log('Running query:', query, 'for company:', companyIdentifier)
