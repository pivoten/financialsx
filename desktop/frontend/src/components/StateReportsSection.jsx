...
export function StateReportsSection({ currentUser }) {
  ...
  useEffect(() => {
    // Simulate loading states from WELLS.dbf
    loadStatesFromWells()
  }, [currentUser])
  ...
  if (states.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>State Reporting</CardTitle>
          <CardDescription>No wells data found or no states identified</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            <Building className="w-12 h-12 mx-auto mb-4 opacity-50" />
            <p>No wells found in WELLS.dbf file.</p>
            <p className="text-sm mt-2">Import wells data to see state-specific reporting requirements.</p>
          </div>
        </CardContent>
      </Card>
    )
  ...
  const currentStateData = states.find(state => state.code === selectedState)
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Reports organized by state based on well locations in your WELLS.dbf file
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {state.wellCount} wells • {state.reports.length} report types
                    </p>
                  </div>
                  <Badge variant="outline">
                    {state.reports.filter(r => r.status === 'overdue').length} overdue
                  </Badge>
                </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {report.type === 'monthly' ? 'Monthly filing' : 
                               report.type === 'quarterly' ? 'Quarterly filing' : 
                               'Annual filing'}
                            </p>
                          </div>
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-2 text-blue-800">
            ...
            <div className="text-sm">
              ...
              <p className="mt-1">
                This interface shows a preview of state-based reporting. The system will automatically:
              </p>
