...
export function UserManagement({ currentUser }) {
  ...
  const canManageUser = (targetUser) => {
    ...
    // Root can manage everyone except other root users
    if (currentUser.is_root) {
      return !targetUser.is_root || targetUser.id === currentUser.id
    }
    
    // Admins can manage read-only users and themselves
    if (currentUser.role_name === 'admin') {
      return targetUser.role_name === 'readonly' || targetUser.id === currentUser.id
    }
    
    // Read-only users can't manage anyone
    return false
  }
                      ...
                      <TableCell>
                        {canManage && currentUser?.permissions?.includes('users.manage_roles') ? (
                          ...
                          <Badge variant={getRoleBadgeVariant(user.role_name)}>
                            {role?.display_name || user.role_name}
                          </Badge>
                        )}
                      </TableCell>
                      <TableCell>
